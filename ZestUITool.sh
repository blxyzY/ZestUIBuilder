...
rm -rf "Output"
mkdir -p "Output"
OUTPUT_IMG="Output/$ROM_TYPE-AB-$android_version-$current_date-ZestUIBuilder.img"
$(pwd)/Tools/mkimage/mkimage.sh "$BASE_DIR" "$OUTPUT_IMG"

if [ -f "release.sh" ]; then
    echo "Uploading build result via release.sh..."
    bash release.sh
else
    echo "release.sh not found, skipping upload."
fi